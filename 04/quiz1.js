...
let page = 1;
const showPagenum = 3;
const endpage = parseInt(todayPhoto.length / 3, 10) + 1;

count.innerHTML = `${page}/${endpage}`;

let str = '';
for (let i = start; i < end; i += 1) {
  str += `<div style ="display:inline-block; margin-right:20px;"><img src="${todayPhoto[i].img}"><div style = width:110px;>${todayPhoto[i].title}</div></div>`;
}
wrap.innerHTML = str;

next.addEventListener('click', function pageUp() {
  page += 1;
  if (page > todayPhoto.length / 3) page = endpage;

  start += showPagenum;
  end += showPagenum;
  if (end >= todayPhoto.length) {
    start = todayPhoto.length - showPagenum;
    end = todayPhoto.length;
  }
...
before.addEventListener('click', function pageDown() {
  page -= 1;
  if (page <= 1) page = 1;
